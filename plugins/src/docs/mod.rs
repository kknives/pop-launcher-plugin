...
async fn query(arg: &str) -> io::Result<(Child, ChildStdout)> {
    let home_dir = env::var("HOME").expect("No $HOME defined");
    let mut child = Command::new("fdfind")
        .arg("-i")
        .arg(arg)
        .arg(format!("{}/Documents", home_dir))
        .arg(format!("{}/Pictures", home_dir))
        .arg(format!("{}/notes", home_dir))
        .arg(format!("{}/books", home_dir))
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()?;
